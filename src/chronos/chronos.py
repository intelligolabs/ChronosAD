...
class MeanScaleUniformBins(ChronosTokenizer):
    ...
    def input_transform(
        ...
        _boundries = self.boundaries.to(scaled_context.device)
        token_ids = (
            torch.bucketize(
                input=scaled_context,
                boundaries=_boundries,
        ...
        if self.config.use_eos_token:
            ...
            token_ids = torch.concat((token_ids, eos_tokens.to(token_ids.device)), dim=1)
            eos_mask = torch.full((batch_size, 1), fill_value=True, device = token_ids.device)
