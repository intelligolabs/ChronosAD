...
def strawberry_dataset_generator(path: str, context: int):
    dataset = pd.DataFrame()

    test = pd.read_csv(f"{path}/Strawberry_TEST.tsv", delimiter='\t', header=None)
    train = pd.read_csv(f"{path}/Strawberry_TRAIN.tsv", delimiter='\t', header=None)
