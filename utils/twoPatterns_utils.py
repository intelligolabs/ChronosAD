...
def twoPatterns_dataset_generator(path: str, context: int):
    dataset = pd.DataFrame()

    test = pd.read_csv(f"{path}/TwoPatterns_TEST.tsv", delimiter='\t', header=None)
    train = pd.read_csv(f"{path}/TwoPatterns_TRAIN.tsv", delimiter='\t', header=None)
